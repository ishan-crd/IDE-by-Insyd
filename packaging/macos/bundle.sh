#!/bin/sh
# Build InsyDE.app and InsyDE-<version>.dmg in target/release/bundle.
#
...
ICONSET="$OUT/InsyDE.iconset" && rm -rf "$ICONSET" && mkdir -p "$ICONSET"
...
iconutil -c icns "$ICONSET" -o "$APP/Contents/Resources/InsyDE.icns"
...
DMG="$OUT/InsyDE-$VERSION.dmg"
STAGE="$OUT/dmg" && rm -rf "$STAGE" && mkdir -p "$STAGE"
cp -R "$APP" "$STAGE/" && ln -s /Applications "$STAGE/Applications"
rm -f "$DMG"
hdiutil create -quiet -volname "InsyDE" -srcfolder "$STAGE" -ov -format UDZO "$DMG"
rm -rf "$STAGE" "$ICONSET"
echo "Built $APP"
echo "Built $DMG"
echo "CLI: ln -sf \"/Applications/InsyDE.app/Contents/MacOS/insy\" /usr/local/bin/insy"
