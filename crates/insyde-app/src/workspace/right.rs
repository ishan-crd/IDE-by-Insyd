...
impl Workspace {
    pub(super) fn render_right(&mut self, t: &Theme, _w: &mut Window, cx: &mut Context<Self>) -> AnyElement {
        let ix = match self.right_tab {
            RightTab::Checks => 0,
            RightTab::Diff => 1,
            RightTab::Files => 2,
        };
        let seg = ui::segmented("right-seg", &["Checks", "Diff", "Files"], ix, t, true, 26., {
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .child(ui::icon_button("pop-right", "popout", 13., t).on_click(cx.listener(|this, _, _, cx| {
                        if let Some(url) = this.wt().and_then(|w| w.pr.as_ref()).map(|p| p.url.clone()) {
                            cx.open_url(&url);
    ...
    fn render_checks(&mut self, t: &Theme, cx: &mut Context<Self>) -> AnyElement {
        ...
        let Some(pr) = ws.pr.clone() else {
            let (files, a, r) = ws.files.iter().fold((0, 0, 0), |acc, f| (acc.0 + 1, acc.1 + f.added, acc.2 + f.removed));
        ...
        for (i, c) in pr.checks.iter().enumerate() {
            ...
            list = list.child(
                div()
                    ...
                    .cursor_pointer()
                    .bg(if c.state == CheckState::Running { t.hover } else { gpui::transparent_black() })
                    .hover(move |s| s.bg(hover))
                    .child(div().size(px(16.)).flex_none().rounded_full().bg(tint).flex().items_center().justify_center().child(icon(glyph, 10., t.palette.white)))
                    .child(
                        div()
                            .flex_1()
                            .min_w_0()
                            .flex()
                            .flex_col()
                            .child(ui::trunc(c.name.clone()).text_size(metrics::TEXT_SM).font_weight(FontWeight::MEDIUM).text_color(t.ink))
                            .child(ui::trunc(c.detail.clone()).text_size(metrics::TEXT_XS).text_color(t.ink_3)),
                    )
                    .child(div().flex_none().text_size(metrics::TEXT_XS).text_color(t.ink_3).child(c.duration.clone()))
                        ...
                        div()
                            ...
                            .child(
                                div()
                                    .flex_1()
                                    .min_w_0()
                                    .child(ui::trunc(format!("#{} · {}", pr.number, pr.branch)).font_weight(FontWeight::SEMIBOLD).text_color(t.ink))
                                    .child(div().mt(px(1.)).text_size(metrics::TEXT_XS).text_color(t.ink_3).child(sub)),
                            ),
                    )
                    .child(
                        div()
                            ...
                            .overflow_hidden()
                            .bg(t.line)
                            .when(ok > 0, |d| d.child(div().flex_grow(1.).flex_basis(gpui::relative(ok as f32 / total as f32)).bg(t.ok)))
                            .when(run > 0, |d| d.child(div().flex_grow(1.).flex_basis(gpui::relative(run as f32 / total as f32)).bg(t.warn)))
                            .when(bad > 0, |d| d.child(div().flex_grow(1.).flex_basis(gpui::relative(bad as f32 / total as f32)).bg(t.err))),
                    ),
            )
            .child(div().id("checks").flex_1().min_h_0().overflow_y_scroll().child(list))
            .child(
                div()
                    .p(px(10.))
                    .border_t_1()
                    ...
                    .items_center()
                    .gap(px(8.))
                    .child(div().flex_1().text_size(metrics::TEXT_XS).text_color(t.ink_3).child(footer))
                    .child(ui::small_button("rerun", "Re-run", t).h(px(28.)).on_click(cx.listener(|this, _, _, cx| this.rerun_checks(cx)))),
            )
            .into_any_element()
    }
    ...
    fn render_diff(&mut self, t: &Theme, cx: &mut Context<Self>) -> AnyElement {
        ...
        let (a, r) = files.iter().fold((0, 0), |acc, f| (acc.0 + f.added, acc.1 + f.removed));
        ...
        for (i, f) in files.iter().enumerate().take(300) {
            ...
            list = list.child(
                div()
                    ...
                    .child(div().flex_1().min_w_0().flex().gap(px(6.)).child(div().flex_none().text_color(t.ink).child(name)).child(ui::trunc(dir).text_color(t.ink_3).text_size(metrics::TEXT_XS)))
                    .child(div().flex_none().text_size(metrics::TEXT_XS).font_weight(FontWeight::MEDIUM).text_color(t.ok).child(if f.binary { "bin".into() } else { format!("+{}", f.added) }))
                    .child(div().flex_none().text_size(metrics::TEXT_XS).font_weight(FontWeight::MEDIUM).text_color(t.err).child(format!("−{}", f.removed)))
                    .on_click(cx.listener(move |this, _, _, cx| this.select_diff_file(p.clone(), cx))),
            );
        }
        let header = div()
            .flex()
            ...
            .pb(px(8.))
            .text_size(metrics::TEXT_SM)
            .child(div().font_weight(FontWeight::MEDIUM).child(format!("{} changed file{}", files.len(), if files.len() == 1 { "" } else { "s" })))
            .child(div().text_color(t.ok).font_weight(FontWeight::MEDIUM).child(format!("+{a}")))
            .child(div().text_color(t.err).font_weight(FontWeight::MEDIUM).child(format!("−{r}")));
        let mut col = div().flex().flex_col().flex_1().min_h_0().child(header);
        if files.is_empty() {
            col = col.child(div().px(px(14.)).text_size(metrics::TEXT_SM).text_color(t.ink_3).child("No changes against the base branch."));
        }
        col = col.child(div().id("diff-files").max_h(px(if patch.is_some() { 180. } else { 2000. })).overflow_y_scroll().child(list));
        if let Some(lines) = patch {
            col = col.child(code_list("patch", lines, t, true).border_t_1().border_color(t.line));
        } else if sel.is_some() {
            col = col.child(div().p(px(14.)).text_size(metrics::TEXT_SM).text_color(t.ink_3).child("Loading diff…"));
        }
    ...
    fn render_viewer(&mut self, t: &Theme) -> AnyElement {
        let Some((path, lines)) = self.wt().and_then(|w| w.viewer.clone()) else {
            return div().p(px(14.)).text_size(metrics::TEXT_SM).text_color(t.ink_3).child("Open a file from the sidebar's Files or Search tab.").into_any_element();
        };
        div()
            .flex()
            .flex_col()
            .flex_1()
            .min_h_0()
            .child(div().px(px(14.)).pb(px(8.)).flex().gap(px(6.)).items_center().child(icon("file", 12., t.ink_3)).child(ui::trunc(path).text_size(metrics::TEXT_SM).text_color(t.ink)))
            .child(code_list("viewer", lines, t, false).border_t_1().border_color(t.line))
            .into_any_element()
    }
}
            ...
            range
                .map(|i| {
                    ...
                    div()
                        ...
                        .whitespace_nowrap()
                        .overflow_hidden()
                        .when(!diff, |d| d.child(div().w(px(38.)).flex_none().text_right().pr(px(10.)).text_color(t.ink_faint).child((i + 1).to_string())))
