...
const CODE_EXT: &[&str] = &[
    "rs", "ts", "tsx", "js", "jsx", "mjs", "py", "go", "swift", "kt", "java", "rb", "ex", "c",
    "cc", "cpp", "h", "hpp", "cs", "php", "vue", "svelte", "dart", "scala", "zig", "lua",
];
    ...
    for (i, s) in dirs.iter().enumerate() {
        if !CONTAINERS.contains(s) {
            ...
            return if i > 0
                && matches!(
                    dirs[i - 1],
                    "packages" | "crates" | "apps" | "services" | "modules"
                ) {
                s.to_string()
            } else {
                capitalize(s)
...
static RE_RS: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?m)^\s*pub(?:\([^)]*\))? (?:async )?(?:unsafe )?(fn|struct|enum|trait|type) ([A-Za-z_]\w*)").unwrap()
});
static RE_TS: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?m)^export (?:default )?(?:async )?(function|class|const|let|interface|type|enum) ([A-Za-z_$][\w$]*)").unwrap()
});
static RE_PY: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?m)^(?:async )?(def|class) ([A-Za-z_]\w*)").unwrap());
static RE_GO: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?m)^(?:func (?:\([^)]*\) )?|type )([A-Z]\w*)").unwrap());
...
static RE_IMPORT_JS: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"(?:from\s+|require\(|import\()\s*['"](\.{1,2}/[^'"]+)['"]"#).unwrap()
});
static RE_MOD_RS: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?m)^\s*(?:pub )?mod ([a-z_]\w*);").unwrap());
static RE_USE_RS: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?m)^\s*(?:pub )?use crate::([a-z_]\w*)").unwrap());
static RE_PY_IMPORT: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?m)^from (\.+\w*|[\w.]+) import").unwrap());
...
static RE_NEXT_METHOD: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?m)^export (?:async )?(?:function|const) (GET|POST|PUT|PATCH|DELETE)\b").unwrap()
});
...
static RE_MERGE_PR: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^Merge pull request #(\d+) from \S+").unwrap());
static RE_SQUASH_PR: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^(.*?) \(#(\d+)\)$").unwrap());
static RE_CONVENTIONAL: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"^(feat|fix|chore|docs|refactor|test|perf|build|ci|style|revert)(\([^)]+\))?!?: ")
        .unwrap()
});
...
fn parse(path: &str, text: &str) -> Parsed {
    let e = ext(path);
    // Test modules describe fixtures, not the product: never mine them.
    let text = match (e, text.find("#[cfg(test)]")) {
        ("rs", Some(i)) => &text[..i],
        _ => text,
    };
    ...
    match e {
        "ts" | "tsx" | "js" | "jsx" | "mjs" | "vue" | "svelte" => imports.extend(
            RE_IMPORT_JS
                .captures_iter(text)
        ...
        "rs" => {
            imports.extend(
                RE_MOD_RS
                    .captures_iter(text)
                    .filter_map(|c| c.get(1))
                    .map(|m| format!("mod:{}", m.as_str())),
            );
            imports.extend(
                RE_USE_RS
                    .captures_iter(text)
                    .filter_map(|c| c.get(1))
                    .map(|m| format!("crate:{}", m.as_str())),
            );
        }
        "py" => imports.extend(
            RE_PY_IMPORT
...
pub fn build(repo: &Path, base: &str, progress: Progress) -> Result<Graph> {
    ...
    for rec in listing.split('\0').filter(|r| !r.is_empty()) {
        ...
        if path
            .split('/')
            .any(|s| SKIP_DIRS.contains(&s) || (s.starts_with('.') && s != ".github"))
        {
            continue;
        }
    ...
    if let Ok(log) = git::run_capped(
        repo,
        &["log", base, "--format=@%ct", "--name-only", "-n", "1500"],
        16 << 20,
    ) {
        let mut ts = 0i64;
        for l in log.lines() {
    ...
    let code: Vec<&(String, u64)> = all
        .iter()
        .filter(|(p, s)| CODE_EXT.contains(&ext(p)) && *s <= MAX_FILE_BYTES && !is_test(p))
        .collect();
    let tests: Vec<&String> = all
        .iter()
        .filter(|(p, _)| CODE_EXT.contains(&ext(p)) && is_test(p))
        .map(|(p, _)| p)
        .collect();
    let docs: Vec<&(String, u64)> = all
        .iter()
        .filter(|(p, s)| (ext(p) == "md" || ext(p) == "mdx") && *s <= MAX_FILE_BYTES)
        .collect();
    ...
    for (i, ((p, size), text)) in ranked.iter().zip(contents.iter()).enumerate() {
        ...
        if i % 40 == 0 {
            progress(
                10 + (i * 40 / paths.len().max(1)) as u8,
                format!("Parsing {} files…", paths.len()),
            );
        }
    }
    ...
    let mut groups: Vec<String> = areas
        .iter()
        .take(MAX_GROUPS - 2)
        .map(|a| a.0.clone())
        .collect();
    ...
    // Docs that don't live inside a feature area get their own group.
    let loose_docs = docs.iter().any(|(p, _)| !groups.contains(&area_of(p)));
    if loose_docs {
        groups.push("Docs".into());
    }
    groups.push("Conventions".into());
    let conv_group = groups.len() - 1;
    let docs_group = groups.iter().position(|g| g == "Docs");
    let group_of = |p: &str| -> usize {
        let a = area_of(p);
        groups
            .iter()
            .position(|g| *g == a)
            .unwrap_or_else(|| groups.iter().position(|g| g == "Other").unwrap_or(0))
    };
    ...
    let root = b.add(
        ...
        project.clone(),
        None,
        format!("Everything the brain knows about {project} on {base}, rolled up by feature area."),
        None,
        0,
        None,
    );
    ...
    for (r, fid, gi) in routes.into_iter().take(60) {
        ...
        let id = b.add(
            ...
            r.clone(),
            Some(gi),
            format!("Endpoint `{r}` defined in {src}."),
            Some(src),
            60,
            None,
        );
    ...
    let log = git::run_capped(
        ...
        &[
            ...
            "600",
            "--name-only",
            "--format=%x1e%H%x1f%s%x1f%b%x1f%ct%x1f",
        ],
        24 << 20,
    )
    .unwrap_or_default();
    ...
    for (i, rec) in records.iter().enumerate() {
        ...
        let (sha_c, subj, body, ts) = (
            f[0].trim(),
            f[1].trim(),
            f[2].trim(),
            f[3].trim().parse::<i64>().ok(),
        );
        let files: Vec<String> = f[4]
            .lines()
            .map(str::trim)
            .filter(|l| !l.is_empty())
            .take(200)
        ...
        let group_for = |files: &[String], b: &Builder| -> usize {
            files
                .iter()
                .filter_map(|p| file_ids.get(p))
                .map(|&id| b.g.nodes[id].group.unwrap_or(0))
                .next()
                .unwrap_or(0)
        };
        let pr = RE_MERGE_PR
            ...
            .map(|c| {
                (
                    c[1].to_string(),
                    body.lines().next().unwrap_or(subj).to_string(),
                )
            })
            .or_else(|| {
                RE_SQUASH_PR
            ...
            && prs < 40
        {
            ...
            let desc: String = body
                .lines()
                .skip(if RE_MERGE_PR.is_match(subj) { 1 } else { 0 })
                .collect::<Vec<_>>()
                .join(" ")
                .chars()
                .take(240)
            ...
            let id = b.add(
                format!("pr:{num}"),
                Kind::Pr,
                format!("PR #{num} · {}", short(&title, 38)),
                Some(gi),
                summary,
                None,
                (body.len() / 4 + 80) as u32,
                ts,
            );
            b.link(hubs[gi], id, EdgeKind::Child);
            for p in &files {
        ...
        if decisions < 25
            && RE_DECISION.is_match(subj)
            && !subj.starts_with("Merge ")
            && !subj.to_lowercase().starts_with("bump")
        {
            decisions += 1;
            let gi = group_for(&files, &b);
            let clean = RE_CONVENTIONAL.replace(subj, "").to_string();
            let clean = RE_SQUASH_PR
                .captures(&clean)
                .map(|c| c[1].to_string())
                .unwrap_or(clean);
            let summary = format!(
        ...
        if i % 50 == 0 {
            progress(
                60 + (i * 20 / n_commits) as u8,
                format!("Mining {prs} PRs…"),
            );
        }
    }
    ...
    let has = |name: &str| {
        all.iter()
            .any(|(p, _)| p == name || p.ends_with(&format!("/{name}")))
    };
    ...
    if all
        .iter()
        .any(|(p, _)| p.contains(".eslintrc") || p.contains("eslint.config"))
    {
        conv(&mut b, "ESLint rules enforced", "eslint config");
    }
    if all
        .iter()
        .any(|(p, _)| p.contains(".prettierrc") || p.contains("prettier.config"))
    {
    ...
    if let Some(Some(ts)) = read_blobs(repo, base, &["tsconfig.json".to_string()])
        .ok()
        .and_then(|v| v.into_iter().next())
        && (ts.contains("\"strict\": true") || ts.contains("\"strict\":true"))
    {
    ...
    if subjects.len() >= 20 {
        let n = subjects
            .iter()
            .filter(|s| RE_CONVENTIONAL.is_match(s))
            .count();
        if n * 100 / subjects.len() >= 50 {
            conv(
                &mut b,
                "Conventional commits",
                &format!("{n} of last {} commits", subjects.len()),
            );
        }
    }
    if !tests.is_empty() {
        let colocated = tests
            .iter()
            .filter(|t| {
                !t.contains("/tests/") && !t.contains("/__tests__/") && !t.starts_with("test")
            })
            .count();
        if colocated * 2 > tests.len() {
            conv(
                ...
                &format!("{colocated} colocated test files"),
            );
        } else {
            conv(
                &mut b,
                "Tests in dedicated test folders",
    ...
    for (name, label) in [
        ...
        (".nvmrc", "Node version pinned (.nvmrc)"),
    ] {
        if root_has(name) {
    ...
    for (gi, &h) in hubs.iter().enumerate() {
        let members: Vec<&Node> =
            b.g.nodes
                .iter()
                .filter(|n| n.group == Some(gi) && n.kind != Kind::Module && n.kind != Kind::Symbol)
                .collect();
...
mod tests {
    ...
    fn skips_rust_tests() {
        let p = parse(
            "src/a.rs",
            "pub fn real() {}\n#[cfg(test)]\nmod t { fn x() { app.post('/fake', h) } }\n",
        );
        assert!(p.routes.is_empty());
        assert_eq!(p.exports, vec!["real"]);
    }
