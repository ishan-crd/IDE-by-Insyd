...
fn main() {
    ...
    let base = a.next().unwrap_or_else(|| insyde_core::git::default_branch(&repo));
    ...
    eprintln!("built in {:?}: {} nodes, {} edges, {} groups {:?}", t.elapsed(), g.nodes.len(), g.edges.len(), g.groups.len(), g.groups);
    for k in Kind::ALL { eprintln!("  {:<10} {}", k.label(), g.nodes.iter().filter(|n| n.kind == k).count()); }
    for n in g.nodes.iter().filter(|n| matches!(n.kind, Kind::Decision | Kind::Convention | Kind::Api | Kind::Pr)).take(20) { eprintln!("  [{}] {}", n.kind.chip(), n.name); }
