...
use insyde_core::agents::{AgentSpec, acp::{AcpSession, Block, Policy}};
use std::sync::Arc;
...
fn main() {
    ...
    while !s.transcript.lock().ready {
        ...
        if t0.elapsed().as_secs() > 120 { eprintln!("timeout starting"); return; }
        std::thread::sleep(std::time::Duration::from_millis(100));
    }
    { let t = s.transcript.lock(); eprintln!("ready in {:?}; models={:?} modes={:?}", t0.elapsed(), t.models, t.modes); }
    s.prompt(vec![Block::Text(prompt)]);
    std::thread::sleep(std::time::Duration::from_millis(300));
    while s.transcript.lock().running {
        let p = s.transcript.lock().permission.clone();
        if let Some(p) = p { eprintln!("permission: {p:?}"); s.answer(None); }
        std::thread::sleep(std::time::Duration::from_millis(100));
    }
    let t = s.transcript.lock();
    for it in &t.items { println!("{it:?}"); }
